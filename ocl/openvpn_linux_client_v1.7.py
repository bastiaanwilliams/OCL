...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent, image_path, message, duration=3000, config=None):
        ...
        self.canvas.create_text(300, 270, text=message, font=("Arial", 12), fill="white", anchor="center")
...
class OpenVPNClientApp:
    ...
    def show_splash_screen(self):
        ...
        splash = SplashScreen(
            self.root,
            splash_image_path,
            "OpenVPN Linux Client version 1.1",
            duration=3000,
            config=self.config
        )
        splash.update()
    ...
    def show_about(self):
        """Open a themed 'About' window with the same style as the app."""
    ...
    #######################################################################
    #                   OPTION 2: Single Loop With Multiple Patterns       #
    #######################################################################
    def run_vpn(self, config_file, username, password):
        """
        Spawns the OpenVPN process and handles multiple potential prompts:
         1) Enter Auth Username
         2) Enter Auth Password
         3) Enter Private Key Password (if a passphrase-protected key is used)
         4) 2FA Challenge
         5) Or times out / sees EOF
        Then it watches for "Initialization Sequence Completed".
        """
        try:
            ...
            patterns = [
                re.compile(r"Enter Auth Username.*:", re.IGNORECASE),     # index=0
                re.compile(r"Enter Auth Password.*:", re.IGNORECASE),     # index=1
                re.compile(r"Enter Private Key Password.*:", re.IGNORECASE),  # index=2
                re.compile(r"CHALLENGE:", re.IGNORECASE),                 # index=3
                pexpect.EOF,                                             # index=4
                pexpect.TIMEOUT                                          # index=5
            ]

            # Loop until we hit EOF or we handle all prompts
            while True:
                index = self.vpn_process.expect(patterns, timeout=60)
                if index == 0:
                    # Enter Auth Username
                    ...
                    # In this example, reuse same 'password' if needed, or prompt user for a separate pass
                    self.vpn_process.sendline(password)
                elif index == 3:
                    # 2FA Challenge
                    self.queue.put("SHOW_MFA")
                    # Wait for user to enter the MFA code in the popup
                    while self.mfa_code is None:
                        time.sleep(0.1)
                    if self.vpn_process and self.vpn_process.isalive():
                        self.vpn_process.sendline(self.mfa_code.strip())
                elif index == 4:
                    # EOF => break out of prompt loop
                    break
                elif index == 5:
                    # TIMEOUT => no prompt was matched for 60 seconds
                    raise Exception("Timed out waiting for next OpenVPN prompt.")
            
            # Now read lines until we see "Initialization Sequence Completed" or an error
            while True:
