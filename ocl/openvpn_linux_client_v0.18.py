...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent, image_path, message, duration=3000, config=None):
        ...
        # Load splash image
        try:
            splash_image = Image.open(image_path).resize((400, 280), Image.Resampling.LANCZOS)
...
class OpenVPNClientApp:
    def __init__(self, root):
        ...
        # Load images
        try:
            self.dark_mode_img = ImageTk.PhotoImage(Image.open("light_mode_icon.png").resize((30,30), Image.Resampling.LANCZOS))
            self.light_mode_img = ImageTk.PhotoImage(Image.open("dark_mode_icon.png").resize((30,30), Image.Resampling.LANCZOS))

            self.connect_button_image_dark = ImageTk.PhotoImage(Image.open("off3.png").resize((160, 70), Image.Resampling.LANCZOS))
            self.disconnect_button_image_dark = ImageTk.PhotoImage(Image.open("on3.png").resize((160, 70), Image.Resampling.LANCZOS))

            self.cover_network_image = ImageTk.PhotoImage(Image.open("cover_network.png").resize((330, 160), Image.Resampling.LANCZOS))


            self.connect_button_image_light = ImageTk.PhotoImage(Image.open("offd_light.png").resize((160, 70), Image.Resampling.LANCZOS))
            self.disconnect_button_image_light = ImageTk.PhotoImage(Image.open("ond_light.png").resize((160, 70), Image.Resampling.LANCZOS))

            self.logo_dark = ImageTk.PhotoImage(Image.open("openvpnlogo.png").resize((290, 72), Image.Resampling.LANCZOS))
            self.logo_light = ImageTk.PhotoImage(Image.open("openvpnlogo_light.png").resize((290, 72), Image.Resampling.LANCZOS))

            # Load background images
            self.bg_image_dark = ImageTk.PhotoImage(Image.open("background_dark.png").resize((330,650), Image.Resampling.LANCZOS))
            self.bg_image_light = ImageTk.PhotoImage(Image.open("background_light.png").resize((330,650), Image.Resampling.LANCZOS))
        except Exception as e:
    ...
    def show_splash_screen(self):
        splash = SplashScreen(self.root, "splash.jpg", "OpenVPN Linux Client BETA version 0.16", duration=3000, config=self.config)
        splash.update()
    ...
    def create_widgets(self):
        ...
        self.toggle_button_theme = tk.Button(
            ...
            relief="flat",
            activebackground=self.current_theme["bg_but"],
        ...
        # Config section
        self.canvas.create_text(30, 160, text="Select your OpenVPN Config File:",
                                font=("Arial", 9), fill=self.current_theme["fg"], anchor="nw")
        ...
        self.canvas.create_text(30, 180, text="Current config:", font=("Arial", 9),
                                fill=self.current_theme["fg"], anchor="nw")
        # Adjusted line: Added 'width' parameter to enable text wrapping
        self.config_text_item = self.canvas.create_text(120, 180, text=self.config_path_var.get(),
                                                        font=("Arial",9), fill=self.current_theme["filetext"], anchor="nw",
                                                        width=180)  # Set width to wrap text

        # Username and Password
        self.canvas.create_text(30, 210, text="\U0001F464 Username:", font=("Arial", 12),
                                fill=self.current_theme["fg"], anchor="nw")
        ...
        self.canvas.create_text(30, 280, text="\U0001F512 Password:", font=("Arial", 12),
                                fill=self.current_theme["fg"], anchor="nw")
        ...
        current_connect_image = self.connect_button_image_dark if self.current_theme == dark_theme else self.connect_button_image_light
        ...
        self.canvas.create_window(85, 365, window=self.toggle_button_connect, anchor="nw")

        # Network Traffic (Centered)
        self.canvas.create_text(165, 485, text="📶 Network Traffic",
                                font=("Arial",12,"bold"),
                                fill="#8ec4e9",
                                anchor="center")

        # Sent and Received Labels
        self.sent_label_item = self.canvas.create_text(100,502, text="📤 Sent: 0.00 MB",
                                                       font=("Arial",9),
                                                       fill=self.current_theme["fg"],
                                                       anchor="center")
        self.received_label_item = self.canvas.create_text(230,502, text="📥 Received: 0.00 MB",
                                                           font=("Arial",9),
                                                           fill=self.current_theme["fg"],
                                                           anchor="center")

        # VPN Status (Centered)
        self.canvas.create_text(165, 532, text="VPN STATUS:", font=("Arial",10),
                                fill=self.current_theme["fg"], anchor="center")
        self.status_text_item = self.canvas.create_text(165, 550, text=" \U0001F513 Not Connected ",
                                                        font=("Arial",12,"bold"),
                                                        fill="#fe7014",
                                                        anchor="center")
    ...
    def update_ui_on_connect(self):
        ...
        self.canvas.itemconfig(self.status_text_item, text=" \U0001F512 Connected ", fill=self.current_theme["highlight_text"])
    ...
    def show_mfa_popup(self):
        ...
        try:
            second_logo_image = Image.open("2fa.png").resize((200, 100), Image.Resampling.LANCZOS)
        ...
        tk.Label(mfa_window, text="\U0001F511 Your 2FA Code:", font=("Arial", 14),
                 bg=self.current_theme["bg"], fg=self.current_theme["fg"]).pack(pady=10)

        mfa_code_var = tk.StringVar()
        mfa_entry = tk.Entry(mfa_window,
                             bg=self.current_theme["bg_input"],
                             fg="orange",
                             font=("Arial", 34), width=6,
                             textvariable=mfa_code_var)
        mfa_entry.pack(pady=10)
        mfa_entry.focus_set()  # Focus on the entry so user can type immediately

        tk.Button(mfa_window, text="Submit", command=submit_mfa,
    ...
    def apply_theme_to_widgets(self):
        ...
        # Update toggle theme button image
        if self.current_theme == dark_theme:
            self.toggle_button_theme.config(image=self.dark_mode_img, bg=self.current_theme["bg"])
            self.toggle_button_theme.image = self.dark_mode_img
        else:
            self.toggle_button_theme.config(image=self.light_mode_img, bg=self.current_theme["bg"])
            self.toggle_button_theme.image = self.light_mode_img

        # Update all text and widgets
        ...
        self.button_browse.config(bg=self.current_theme["button_bg"], fg=self.current_theme["button_fg"])
        ...
        # Update connect/disconnect button based on connection
        if self.connected:
            if self.current_theme == dark_theme:
                new_image = self.disconnect_button_image_dark
        ...
        self.toggle_button_connect.config(image=new_image, bg=self.current_theme["bg"])
        self.toggle_button_connect.image = new_image
