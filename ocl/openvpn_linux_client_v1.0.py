#!/usr/bin/env python3
"""
OpenVPN Linux Client (Bundled OpenVPN)
...
# 1. Adjust the path to the local OpenVPN binary
#    We'll detect platform or just default to "openvpn" in bin/
import platform
...
# 2. Construct the path to the bundled OpenVPN
...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent, image_path, message, duration=3000, config=None):
        ...
        self.center_on_primary_monitor(400, 320)
...
class OpenVPNClientApp:
    def __init__(self, root):
        ...
        try:
            self.dark_mode_img = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "light_mode_icon.png")).convert("RGBA")
                .resize((30, 30), Image.Resampling.LANCZOS)
            )
            self.light_mode_img = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "dark_mode_icon.png")).convert("RGBA")
                .resize((30, 30), Image.Resampling.LANCZOS)
            )

            self.connect_button_image_dark = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "on3.png")).convert("RGBA")
                .resize((70, 70), Image.Resampling.LANCZOS)
            )
            self.disconnect_button_image_dark = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "off3.png")).convert("RGBA")
                .resize((70, 70), Image.Resampling.LANCZOS)
            )

            self.cover_network_image = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "cover_network.png")).convert("RGBA")
            ...
            self.logo_dark = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "openvpnlogo.png")).convert("RGBA")
                .resize((290, 72), Image.Resampling.LANCZOS)
            )
            self.logo_light = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "openvpnlogo_light.png")).convert("RGBA")
                .resize((290, 72), Image.Resampling.LANCZOS)
            )

            self.bg_image_dark = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "background_dark.png")).convert("RGBA")
                .resize((330, 650), Image.Resampling.LANCZOS)
            )
            self.bg_image_light = ImageTk.PhotoImage(
                Image.open(os.path.join(BASE_DIR, "images", "background_light.png")).convert("RGBA")
                .resize((330, 650), Image.Resampling.LANCZOS)
            )
        except Exception as e:
    ...
    def show_splash_screen(self):
        splash = SplashScreen(self.root, os.path.join(BASE_DIR, "images", "splash.jpg"), 
                             "OpenVPN Linux Client BETA version 0.20", duration=3000, config=self.config)
        splash.update()
    ...
    def start_vpn(self):
        # Reset MFA code each time we start a fresh connection
        self.mfa_code = None
    ...
    def run_vpn(self, config_file, username, password):
        try:
            # <-- BUNDLED OPENVPN CHANGE:
            # Use the local OpenVPN binary (BUNDLED_OPENVPN_PATH) 
    ...
    def stop_vpn(self):
        if self.vpn_process and self.vpn_process.isalive():
            try:
                self.vpn_process.sendcontrol('c')  # Graceful shutdown
    ...
    def show_mfa_popup(self):
        ...
        try:
            second_logo_image = (
                Image.open(os.path.join(BASE_DIR, "images", "2fa.png"))
                .convert("RGBA")
